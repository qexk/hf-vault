...
[<AutoOpen>]
module private __ThemeThreadsImpl__ = begin
  ...
  { let! realm = Option.get <!> Machine.Pervasives.realm in
    ...
    let! threads = Db.``select all threads from theme`` realm theme limit offset
                |> Freya.fromJob
    in
    let inline toDto t =
      Domain.List.dto_<_, Domain.Thread>()
      |> snd
      <| t
    in
    let json = threads
            |> toDto
            |> Dto.List.jsonEncoder<Dto.Thread>
...
type ThreadsMachine = Threads with
